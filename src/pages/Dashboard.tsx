...
const COLORS = [
  '#6366f1', '#8b5cf6', '#0ea5e9', '#14b8a6', '#f59e0b', '#10b981', '#f97316',
];
      ...
      {meeting.video_link && (
        <a
          ...
          className="shrink-0 inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg
            bg-gradient-to-br from-indigo-500 to-violet-600 text-white hover:from-indigo-600 hover:to-violet-700 transition-all shadow-sm shadow-indigo-200"
        >
...
export default function Dashboard() {
  ...
  const [stats, setStats] = useState<DashboardStats>({
    totalRevenue: 0, pipelineValue: 0, activeDeals: 0, newLeads: 0,
    winRate: 0, totalContacts: 0, totalCompanies: 0, overdueTasks: 0,
  });
  ...
  const fetchIdleLeads = async () => {
    ...
    if (!error && typeof count === 'number') setIdleLeadsCount(count);
  };
    ...
    <div className="space-y-8 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Welcome back! Here's your company overview.</p>
        </div>
      ...
      {role !== 'user' && (
        <div className="rounded-xl border border-amber-200/60 bg-amber-50/30 p-4 space-y-3">
          ...
          {idleLeadsCount === 0 && stats.overdueTasks === 0 && (
        ...
        {dealsByStage.length > 0 ? (
          <div className="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-[0_1px_6px_rgba(0,0,0,0.04)]">
            <div className="mb-6">
              ...
              <p className="text-xs text-slate-400 mt-0.5">Distribution across pipeline</p>
            </div>
            <div className="h-72">
        ...
        {dealsByStage.length > 0 ? (
          <div className="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-[0_1px_6px_rgba(0,0,0,0.04)]">
            ...
            <ResponsiveContainer width="100%" height={260}>
              <PieChart>
                <Pie data={dealsByStage} cx="50%" cy="50%"
      ...
      <div className="bg-gradient-to-r from-indigo-500/5 via-violet-500/10 to-indigo-500/5 rounded-2xl p-6 border border-indigo-100">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-3">
            <a href="/leads"
              className="inline-flex items-center justify-center px-4 py-2 text-sm font-semibold rounded-xl bg-gradient-to-br from-indigo-500 to-violet-600 text-white hover:from-indigo-600 hover:to-violet-700 transition-all shadow-sm shadow-indigo-200">
              Add Lead
            </a>
            <a href="/pipeline"
              className="inline-flex items-center justify-center px-4 py-2 text-sm font-semibold rounded-xl border border-slate-200 bg-white hover:bg-slate-50 hover:border-indigo-200 hover:text-indigo-600 transition-all">
              View Pipeline
            </a>
          </div>
        </div>
