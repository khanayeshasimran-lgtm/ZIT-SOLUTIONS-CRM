 ...
 * src/pages/Auth.tsx
 *
 * DAY 1 FIX: Removed dead code in handleSubmit.
 *
 * Original bug: after signUp(), the code called supabase.auth.getUser()
 * to capture `newUser` and then did absolutely nothing with it — the
 * variable was declared, assigned, and immediately discarded.
 * This was a leftover from an incomplete role-assignment implementation.
 *
 * What's here now: signup calls signUp(), shows a success toast, and
 * navigates to /dashboard. Role assignment happens server-side via the
 * Supabase auth trigger that creates the profile row — not here.
 *
...
export default function Auth() {
  ...
  const validateForm = () => {
    ...
    if (!isLogin) {
      ...
      catch (e) { if (e instanceof z.ZodError) newErrors.fullName = e.errors[0].message; }
    }
    ...
    try {
      if (isLogin) {
        ...
        if (error) {
          ...
          // FIX: Removed the dead `supabase.auth.getUser()` call whose result
          // was never used. Profile creation is handled by the database trigger
          // on auth.users insert — no client-side action needed here.
          toast({ title: 'Account created!', description: 'Welcome to Z IT Solutions CRM.' });
          navigate('/dashboard');
        }
      }
    } finally {
        ...
        <div className="relative flex items-center justify-center w-full p-12">
          <div className="max-w-md text-center">
            ...
            <div className="space-y-5 text-left bg-primary-foreground/5 backdrop-blur-sm rounded-xl p-6 border border-primary-foreground/10">
              {[
                { icon: Users,     text: 'Manage leads and sales pipeline' },
                { icon: Building2, text: 'Track contacts and companies' },
                { icon: Briefcase, text: 'Monitor projects and tickets' },
        ...
        <div className="w-full max-w-md animate-fade-in relative">
          ...
          <div className="lg:hidden relative mb-5 overflow-hidden rounded-3xl bg-gradient-to-br from-primary via-primary/90 to-primary/90 p-6 text-primary-foreground shadow-xl">
            ...
            <div className="relative z-10">
              ...
              <h1 className="text-2xl font-bold text-center tracking-tight">Z IT Solutions CRM</h1>
              <p className="text-center text-sm text-primary-foreground/80 mt-2">Simple. Secure. Sales-focused.</p>
            </div>
          </div>
